...
export default function ShapeList() {
  ...
  useEffect(() => {
    const loadShape = async () => {
      const response = await FetchData(`pokemon-shape`);
      console.log(response);
      setShapeList(response.results);
    };
    loadShape();
  }, []);
  ...
  return (
    <div className="max-w-6xl mx-auto p-8">
      <div className="mb-8">
        <h2 className="text-3xl font-bold text-slate-900 mb-4">Shapes</h2>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
        {shapeList.map((shape, index) => (
          <Link
            to={`/pokemon-shape/${shape.name}`}
            key={index}
